...
export class ActaDevolucionComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  fechaHoy = new Date().toLocaleDateString('es-CO', { year: 'numeric', month: 'long', day: 'numeric' });
  ...
  cargarActasPendientes(): void {
    ...
    this.inventarioService.obtenerActas().subscribe({
      next: (actas) => {
        ...
        this.actasPendientes = actas.filter(acta => 
  ...
  registrarDevolucion(): void {
    ...
    this.inventarioService.registrarDevolucion(this.actaSeleccionada!.id!, formData).subscribe({
      next: (response) => {
